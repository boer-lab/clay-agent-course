...
```
Visit [the website you handed it]. In one or two words, what industry is
this company in? Return only the industry name.
```

That is all it is. A question, with a company's website dropped into it. When you ran it on stripe.com in Lesson 3, that sentence is what did the work.
...
One thing to know before you change the question. It is not a private copy sitting inside your workflow. It is a setting Clay keeps in your workspace, and research steps share it. So if you have more than one research step, or more than one workflow, changing the question in one place can change it in the others at the same time. And if anyone else builds in the same Clay workspace as you, their edits reach your step too — so if a question you just set stops sticking, that is them, not you.

Nothing warns you when that happens. The run finishes, says it is very sure, and hands you real sources for the answer it gave. They are real. They just answer a question you did not ask.

So after you change the question, run it and read the question back off the finished run rather than off the step's settings. The settings tell you what is saved now; the run tells you what actually executed. Then look at the answer's shape: is it answering what you just wrote? And if a workflow you were not even touching starts coming back with something else, that is this, not a mistake you made.
...
- The question lives on a tool object Clay keeps at the workspace level, so a newly built research step attaches to the one already there rather than getting its own copy. Verified live on July 19, 2026: a workflow whose step was still named "Research company industry" was in fact asking a pricing question, set by an edit made in a different workflow.
